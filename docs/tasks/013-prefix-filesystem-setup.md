...
## Fluxo TDD
- [x] **Red**: Escrever teste que assevera que, após chamar a criação do prefix, os subdiretórios esperados (`root/`, `data/`, etc.) existem no sistema de arquivos.
- [x] **Green**: Implementar `create_dir_all` e a lógica de verificação de existência.
- [x] **Refactor**: Adicionar suporte a caminhos configuráveis e validação de permissões.
