...
## Todos
- [x] Implement `Prefix::reset(&self)` to clean the data directory
- [x] Implement `run-android-app reset <package>` CLI command
- [x] Redirect sandboxed process output to a log file in `<prefix>/logs/`
- [x] Implement `run-android-app logs <package>` to tail the log file
